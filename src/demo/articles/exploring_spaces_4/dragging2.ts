...
const unitDiskToUIDisk = ([x, y]) =>
  [(x * RADIUS) / 2, (y * RADIUS) / 2] as Vec2;
const unitDiskToHemisphereStereo = ([x, y]) => {
  const h = x ** 2 + y ** 2;
  return [(2 * x) / (h + 1), (2 * y) / (h + 1), -(h - 1) / (h + 1)];
};
const hemisphereToUnitDiskStereo = ([x, y, z]): Vec2 => [
  x / (1 + z),
  y / (1 + z),
];
...
const twoUnitDisksToSphere = ({ disk, v }: { disk: 0 | 1; v: Vec2 }) => {
  const hemi = unitDiskToHemisphere(v);
  return disk === 0 ? hemi : [-hemi[0], -hemi[1], -hemi[2]];
};
const sphereToTwoUnitDisks = ([x, y, z]): { disk: 0 | 1; v: Vec2 } => ({
  disk: z > 0 ? 0 : 1,
  v:
    z > 0
      ...
      try {
        ...
        if (shouldSetPos) {
          ...
          if (this.disk === 0 && this.pos[1] >= 0) {
            const ypart = [100 + hh * 100, 100 + hh * 100] as [number, number];
            const xpart = [
              (ww * 50 - 50) * (1 - hh),
              (-ww * 50 + 50) * (1 - hh),
            ] as [number, number];
