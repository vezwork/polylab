import { pairs } from "../../lib/structure/Iterable.js";
import { distance as dist } from "../../lib/math/Vec2.js";
import { areIntersecting } from "../../lib/math/Line2.js";
import { mergeAndSortLines } from "../../lib/math/Line2MergeAndSort.js";
...
c.addEventListener("mousedown", (e) => {
    ...
    const addPoint = [e.offsetX, e.offsetY];
    const isLineEmpty = newestPoint?.[0] === undefined;
    if (isLineEmpty ||
        (addPoint[0] > newestPoint[0] &&
            lines.every((l) => l === newLine || !areIntersecting([newestPoint, addPoint], l))))
        newLine?.push(addPoint);
});
window.addEventListener("keydown", (e) => {
    if (e.key === "n" && (lines.at(-1)?.length ?? 0) > 0)
        lines.push([]);
});
...
function draw() {
    ...
    for (const [la, lb] of pairs(mergedAndOrderedLines)) {
        ...
        if (la.length === 0 || lb.length === 0)
            continue;
        for (const p1 of la) {
            const closest = lb.reduce((acc, p) => dist(p1, p) < acc.dist ? { dist: dist(p1, p), point: p } : acc, { dist: Infinity, point: null });
            if (closest.point) {
