import { add, assign, Vec2 } from "../../../lib/math/Vec2.js";
...
function draw() {
  ...
  if (
    squares[sqrSpaceBot]?.[sqrSpaceL] === "ground" ||
  ...
  } else player.spd[1] += GRAVITY;

  if (keysDown["ArrowLeft"]) {
    if (squares[sqrSpaceVM][sqrSpaceL] === "air") player.spd[0] = -1;
    else player.spd[0] = 0;
  } else if (keysDown["ArrowRight"]) {
    if (squares[sqrSpaceVM][sqrSpaceR] === "air") player.spd[0] = 1;
    else player.spd[0] = 0;
  } else player.spd[0] = 0;
  ...
  for (let y = 0; y < squares.length; y++) {
    ...
    for (let x = 0; x < row.length; x++) {
      const entry = row[x];
      if (entry === "ground") ctx.fillRect(x * 40, y * 40, 40, 40);
    }
  }
