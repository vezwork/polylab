...
export const twoUnitDisksToSphere = ({ disk, v }: { disk: 0 | 1; v: Vec2 }) => {
  const hemi = unitDiskToHemisphere(v);
  return disk === 0 ? hemi : [-hemi[0], -hemi[1], -hemi[2]];
};
export const sphereToTwoUnitDisks = ([x, y, z]): { disk: 0 | 1; v: Vec2 } => ({
  disk: z > 0 ? 0 : 1,
  v:
    z > 0
...
const makeCircleAndClone = (
  ...
  svg.append(circle);
  if (draggable) circle.classList.add("draggable");
  ...
  svg.append(clone);
  if (draggable) clone.classList.add("draggable");
...
export const createDraggableDot = (
  svg1,
  svg2,
  setPosCallback = (x, y) => {}
) => {
