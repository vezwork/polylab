export const push = (...starts: any[]) => {
  ...
  const sortedAnds = reachableAnds.sort((and1, and2) =>
    andTrack.get(and1)?.has(and2) ? -1 : 1
  );

  const visitedNodes = new Set(starts);
  // queue entry's [0] and [1] are often redundant because Edge[1] = entry[1], but Edge can be null.
  let queue = starts.map((node) => [null, node]) as [Edge | null, any][];
  ...
  for (const and of [[], ...sortedAnds]) {
    ...
    while (queue.length > 0) {
      ...
      for (const [f, from, to] of and) {
        if (and.length > 0) console.log("get ands", from, to);
        f(from, to);
      }
      ...
      for (const forward of get(graph, fromNode)) {
        ...
        if (forwardAnd === undefined) {
          // don't revisit nodes
          if (visitedNodes.has(toNode)) continue;
