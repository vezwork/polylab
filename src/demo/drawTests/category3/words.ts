...
export const connectivity = (
  ...
  function matchPrefix(input, t) {
    for (let j = 0; j < t.length; j++) {
      if (input[j] === undefined || !eqEntry(input[j])(t[j])) return false;
    }
    return true;
  }
  ...
  const isos = (words) => {
    ...
    for (const a of words) {
      for (const b of words) {
        if (eq(inverse(a))(b)) out.push(a);
      }
    }
    return out;
  };

  const queryConnectedness = (words, path) =>
    consume(path, words) === path.length;

  const _addEdge = (words, path) => {
    if (path.length === 0 || words.some(eq(path))) return [...words];
    else return [...words, path];
  };
  const _addEdges = (words, ...paths) => {
    let wordsOut = [...words];
    for (const path of paths) wordsOut = _addEdge(wordsOut, path);
    return wordsOut;
  };
  ...
  const _addPath = (words, path) => {
    ...
    if (strippedPath.length === 2) {
      return _addEdges(
        ...
        [strippedPath[0]],
        [strippedPath[1]],
        inverse([strippedPath[0]]),
        inverse([strippedPath[1]])
      );
    }
    return _addEdges(words, strippedPath);
  };

  const edge = (words, edgeData, path = undefined) =>
    path
      ? _addPath(_addPath(words, path), inverse(path))
      : _addEdges(words, edgeData, inverse(edgeData));

  const pathedSedge = (words, edgeData, path) =>
    queryConnectedness(words, path)
      ...
      : // add fresh inverse edge (left) and pathed edge (right)
        _addPath(
          ...
          _addPath(words, inverse(edgeData)),
          inverse([...path, ...edgeData])
        );
  const sedge = (words, edgeData1, edgeData2) =>
    _addEdges(words, inverse(edgeData1), inverse(edgeData2));
