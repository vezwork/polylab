...
let state = {
    ...
    render() {
        ...
        const zoom2 = 2 ** (zoom % 1);
        let i = Math.floor(-lines.length / 2);
        for (const line of lines) {
            const pos = f(x + i * zoom2) * width;
            const size = bump(pos);
            const value = (i + Math.trunc(cx)) / 2;
            let s = 1 - Math.abs(value % 1);
            const closeness = zoom2 - 1 / s;
            const scurve = (s) => (Math.tanh(6 * s - 3) + 1) / 2;
            const scalerThing = scurve(closeness + 1);
            line?.setAttribute("transform", `translate(${pos.toFixed(3)} 0) scale(${(size * scalerThing).toFixed(3)})`);
            i++;
            line.querySelector("text").textContent = (value /
                2 ** Math.trunc(zoom)).toFixed(1);
        }
    },
};
function draw() {
