...
const topLeftVec2 = (sink) => ({
    vec2: [sink.left, sink.top],
    sink,
});
const topRightVec2 = (sink) => ({
    vec2: [sink.right, sink.top],
    sink,
});
...
export class ContainerSink extends Sink {
    ...
    calculateChildLines() {
        this.cachedLines = mergeAndSort(map(this.children, (s) => [topLeftVec2(s)])).map((line) => line.map((v) => v.sink));
    }
    // SIBLING STUFF
    leftSiblingOf(child) {
