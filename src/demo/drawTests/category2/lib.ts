...
const edgeMap = new Map<unknown, Edge[]>();
const reverseEdgeMap = new Map<unknown, Edge[]>();
const edge = (
  from: unknown,
  to: unknown,
...
const applyPath = (node, path: REdge[]) => {
  let cur = node;
...
const mapSymbolEdges = (map, symbol, createFromEdge) =>
  new Map(
    map.get(symbol).map((edge) => [edge, contain(() => createFromEdge(edge))])
  );
...
const create = (symbol, visit?: { visitNode: unknown; visitEdge: REdge }) => {
  if (t.has(symbol)) t.set(symbol, t.get(symbol)! + 1);
  else t.set(symbol, 0);
  const createNode: any = { t: t.get(symbol) };
...
tests(
  "3: GR and inv(GR)",
  ["GR = inv(RG) RR", applyPath(g0, [GR]) === applyPath(g0, [inv(RG), RR])],
  [
    "inv(GR) = inv(RR) RG",
    applyPath(r0, [inv(GR)]) === applyPath(r0, [inv(RR), RG]),
  ],
  ["RR inv(RR) = id", applyPath(r0, [RR, inv(RR)]) === r0],
  ["GR inv(GR) = id", applyPath(g0, [GR, inv(GR)]) === g0]
);
...
tests(
  "4: XX 2-cycle",
  ["XX = inv(XX)", applyPath(x0, [XX]) === applyPath(x0, [inv(XX)])],
  ["XX XX = id", x0, applyPath(x0, [XX, XX]) === x0]
);
...
function tests(title, ...items) {
  ...
  if (failures === 0)
    console.log(
      `%c✅ ${title}`,
      "font-style: bold; font-size: 16px; color: green;"
    );
  else
    console.log(
      `%c❌ ${title}`,
      "font-style: bold; font-size: 16px; color: red;"
    );
}
