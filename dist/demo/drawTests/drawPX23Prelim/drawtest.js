import { besideD, centerBesideD, centerOverD, justBesideD, lineD, overD, padD, pathD, render, scaleD, textD, translateD, tTree, } from "../../../lib/draw/draw3.js";
import { inv, lerp } from "../../../lib/math/CtxTransform.js";
...
const radical = (slot) => besideD(lrad, overD(lx, padD(slot, [5, 5])));
const sradical = (slot) => radical(scaleD(slot, [3 / 4, 3 / 4]));
const lad = (slot) => overD(lx, padD(slot, [5, 5]));
const slad = (slot) => lad(scaleD(slot, [3 / 4, 3 / 4]));
const frac = (slot1, slot2) => centerOverD(padD(slot1, [5, 5]), lad(slot2));
const exp = (slot1, slot2) => centerBesideD(slot1, scaleD(translateD(slot2, [0, -15]), [3 / 4, 3 / 4]));
...
const ΣNote = centerBesideD(centerOverD(tD("∞"), tD("Σ", 100), padD(justBesideD(tD("n"), tD("="), tD("1")), [10, 10])), frac(tD("1"), exp(tD("n"), tD("s"))));
const placeholder = translateD(lineD([
    [0, 0],
    [200, 10],
]), [0, 0]);
...
let t = 0;
let start = inv(tTree(φ)[0].t); //idT;
const target = inv(tTree(φ)[1][1][1][0][0][0][0].t);
function draw() {
