...
const sexp = (n, as) => as.map((ai) => ai ** n);
const pmul = (as, bs) => as.map((ai, i) => ai * bs[i]);
const angleOf = ([x, y]) => Math.atan2(y, x);
const add = (v1, v2) => [v1[0] + v2[0], v1[1] + v2[1]];
const lerp = (start, end, t) => add(start, smul(t, sub(end, start)));
const merp = (start, end, t) =>
  pmul(start, sexp(t, pmul(end, sexp(-1, start))));
...
const dhp = ([x, y, z]) => ctx.fillRect(x / z, y / z, 0.1, 0.1);
...
function draw() {
  ...
  const tt = ((t / 200) % 9) - 4;
  const lps = factorPs.map((p, i) => merp(p, targetFactorPs[i], tt));
  ...
  ctx.fillStyle = "red";
  ctx.fillRect(...ps[0].slice(0, 2), 3, 3);
  ctx.fillStyle = "orange";
  ctx.fillRect(...ps[1].slice(0, 2), 3, 3);
  ctx.fillStyle = "OrangeRed";
  ctx.fillRect(...ps[2].slice(0, 2), 3, 3);
  ctx.fillStyle = "tomato";
  ctx.fillRect(...ps[3].slice(0, 2), 3, 3);

  ctx.fillStyle = "blue";
  inPs.forEach((p) => ctx.fillRect(...p.slice(0, 2), 3, 3));
  ctx.fillStyle = "green";
  targetPs.forEach((p) => ctx.fillRect(...p.slice(0, 2), 3, 3));
  ctx.restore();
