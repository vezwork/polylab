...
export const level = (a) => {
  ...
  for (let i = 0; i < l(a); i++) {
    if (at(a, i) !== 0n) return i + 1;
  }
  return 0;
};
export const normalizeRadix = (a) =>
  framed(a, Math.max(l(a), 0), Math.min(r(a), 0));
...
export const sub = (a, b, base) => {
  ...
  if (!a.isNeg && b.isNeg) return mainadd(a, b, base);
  if (a.isNeg && !b.isNeg) return neg(mainadd(a, b, base));
};
