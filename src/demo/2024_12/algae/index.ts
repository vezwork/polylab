...
const onSet = new SetMap<Val, Function>();
const addOnSet = (ob: Val, f: Function) => onSet.add(ob, f);
const getOnSet = (ob: Val) => onSet.get(ob) ?? [];

const set = (ob: Val) => (v: any) => {
  if (ob.val !== undefined) return;
  ob.val = v;
  for (const f of getOnSet(ob)) f(v);
};
const eq = (ob1: Val, ob2: Val) => {
  addOnSet(ob1, set(ob2));
  addOnSet(ob2, set(ob1));
};
// when ob1 is set, set ob2
const toEq = (ob2: Val, ob1: Val) => {
  addOnSet(ob1, set(ob2));
};
const all = (...obs: Val[]) => {
  const allOb = {};
  ...
  for (const ob of obs) addOnSet(ob, afterAll);
  return allOb;
};
const map = (ob: Val) => (f: (v: any) => any) => {
  const mappedOb = {};
...
const add = (...obs: Val[]) =>
  mapAll(...obs)((...vs) => vs.reduce((s, v) => s + v));
const mul = (...obs: Val[]) =>
  mapAll(...obs)((...vs) => vs.reduce((s, v) => s * v));
const min = (...obs: Val[]) => mapAll(...obs)(Math.min);
const max = (...obs: Val[]) => mapAll(...obs)(Math.max);
const sub = (ob1: Val, ob2: Val) => mapAll(ob1, ob2)((v1, v2) => v1 - v2);
const div = (ob1: Val, ob2: Val) => mapAll(ob1, ob2)((v1, v2) => v1 / v2);
...
const group = (...ds: Drawable[]): Drawable => {
...
const stackH = (...ds: Drawable[]): Drawable => {
  ...
  const w = map(all(...ds.map((d) => d.w)))((ws) =>
    ws.reduce((acc, w) => acc + w, 0)
  );
  const x = ds[0].x;
  for (let i = 0; i < ds.length - 1; i++) {
    toEq(
      ds[i + 1].x,
      map(all(ds[i].x, ds[i].w))(([x, w]) => w + x)
    );
    toEq(
      ds[i].x,
      map(all(ds[i + 1].x, ds[i].w))(([x, w]) => x - w)
    );
  }
  return {
    w,
