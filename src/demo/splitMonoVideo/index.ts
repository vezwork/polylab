...
// A custom server is required to serve this lib so that video
// can be recorded.
// see `canvas-capture-deno.ts` in the root of src
declare const CanvasCaptureLib;
...
const TOPLEFT = [120, 20] as Vec2;
const BOTTOMRIGHT = [320, 220] as Vec2;
...
const lchVals = (x, y, greyish): [number, number, number] => {
...
const getColor = (x, y, greyish) =>
  lchValsToCssLchString(lchVals(x, y, greyish));
  ...
  if (t < STAGE2) {
    for (let x = TOPLEFT[0]; x < BOTTOMRIGHT[0]; x += DENSITY) {
      for (let y = TOPLEFT[1]; y < BOTTOMRIGHT[1]; y += DENSITY) {
        ...
        if (y > CENTER[1] && distance([x, y], CENTER) > MAXDIST) break;
...
const endPos = (x, y) =>
  [LLEFT + (lchVals(x, y, 1)[0] / 100) * (LRIGHT - LLEFT), LY] as [
    number,
    number
  ];
...
c.addEventListener("click", (e) => {
  ...
  const y = e.offsetY;
  console.log(
    `${x}, ${y} => %c ${getColor(x, y, 1)}`,
    `background:${getColor(x, y, 1)}; color: white;`
  );
