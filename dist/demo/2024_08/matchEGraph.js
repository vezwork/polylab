...
const eClassMatches = (patternNode, eClass) => [...items(eClass)].flatMap(eNodeMatches(patternNode));
...
const successeNodeMatch = (patternNode, eNode) => {
    const childrenMatches = patternNode.children.map((p, i) => eClassMatches(p, eNode.children[i]));
...
const matchRule = ({ from, to }) => (eClass) => {
...
const enactRuleOnMatches = (matches) => matches.map(({ c, to, matchCombos }) => matchCombos.map((o) => to(o, c)));
