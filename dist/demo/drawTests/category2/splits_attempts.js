...
import { applyPath, create, edge, edgeData, inv, } from "./lib.js";
export const splits = new Map();
export const splitEdge = (...froms) => (to) => {
    // add all edges
            ...
            if (split) {
                ...
                if (areSplitSourcesVisited) {
                    if (visitedSymbols.has(to.symbol)) {
                        if (!visitedThings.has(to))
                            toVisitOutside.push(to);
                    }
                    else
                        toVisit.push(to);
...
export function* walkSpec2(start) {
    const edgeVisits = new Map();
    const nodeEdgeVisits = new Map();
    const visitedThings = new Set([start]);
    const toVisit = [start];
    while (toVisit.length > 0) {
        const cur = toVisit.shift();
        visitedThings.add(cur);
        for (const [e, { get }] of cur.to.entries()) {
            ...
            if (split) {
                const splitSaturation = split.map((se) => nodeEdgeVisits.get(to)?.has(se));
                // console.log(
...
*/
function spreadAndReturn(iterable) {
...
const splitOuts = [...applyPath(b, [ebc1]).from.entries()].map(([e]) => (e.path() ? e.path().slice(0, -1) : [e]).map(edgeData));
const splitSourcesReachability = splitOuts.map((es) => es.every((e) => reachableEdges.includes(e)));
...
const reachableSplitSources = (cur, s) => {
...
export function* walk2(start) {
    const workingSplits = new Map();
    const visitedThings = new Set();
    const toVisit = [[start, null]];
    while (toVisit.length > 0) {
        const [cur, visitEdge] = toVisit.shift();
        if (visitedThings.has(cur))
            continue;
        visitedThings.add(cur);
        yield visitEdge;
        for (const [e, { get }] of cur.to.entries()) {
            ...
            if (split) {
                ...
                if (ws !== undefined) {
                    ...
                    if (allWorkingSplitSourcesVisited) {
                        ...
                        for (const [rsse] of ws)
                            workingSplits.get(to)?.delete(rsse);
                        if (workingSplits.get(to)?.size === 0)
                            workingSplits.delete(to);
                    }
                }
                else {
                    // create working split by calculating reachable split sources
                    const rssFlags = reachableSplitSources(cur, splits.get(e));
                    const rss = split.filter((v, i) => rssFlags[i]);
                    console.log("check working split", splits.get(e), reachableSplitSources(cur, splits.get(e)));
                    if (rss.length === 0) {
                        console.log("0");
...
const prettyPrintEdge = (e) => {
    ...
    if ("isReverse" in e) {
        if (split)
            return `${data.from.description}<s-${data.to.description}`;
        else
            return `${data.from.description}<-${data.to.description}`;
    }
    else {
        if (split)
            return `${data.from.description}-s>${data.to.description}`;
