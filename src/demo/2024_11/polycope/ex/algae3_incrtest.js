...
//
// in general, the system assumes things are sync'd (because of the
// differential nature of group setting)
//
// This could be addressed with an init that sets multiple things at the same time
// or incremental maintenance when adding rels (also nice for changing rels at run time)
...
const draw = () => {
  ...
  for (const ob of Obs) {
    ctx.fillStyle = focus === ob ? "red" : "black";
