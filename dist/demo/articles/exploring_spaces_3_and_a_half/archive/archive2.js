...
// constraints:
// f(n,zoom) = f(n+1, zoom)
// TODO: f(n,x*zoom) = f(x*n, zoom)
// current: f(integer)
const f = (n, zoom) => approachOne(zoom)(Math.abs(n)) * sign(n);
let state = {
    ...
    render() {
        ...
        const zoom2 = zoom % 1;
        const z = ((1 - zoom2) * 1) / 5 + (zoom2 * 9) / 25;
        ...
        for (const line of lines) {
            ...
            //value / 2 ** Math.trunc(zoom)
            line.querySelector("text").textContent = (x + i).toFixed(2);
            i++;
        }
    },
};
