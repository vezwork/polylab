// 2026 Feb 4
// working "projective" version as long as
// a->b is towards ppx and a->c is towars ppy
// in other words, works in the orange quadrant
//
// can solve that by quadrant cases
// there is also a lot of numerical instability
// can be solved for casing for extreme values
import {
  v,
  sub,
  add,
...
const proj =
  ...
  ([u, v]) => {
    ...
    const linex = [lerp(a, ppx, f(fi(dix) * u)), ppy];
    const liney = [lerp(a, ppy, f(fi(diy) * v)), ppx];
...
function draw() {
  ...
  for (let x = -60; x < 60; x += 1) {
    for (let y = -60; y < 60; y += 1) {
      ...
      ctx.beginPath();
      ctx.moveTo(...proj(...ps)([x, y]));
      ctx.lineTo(...proj(...ps)([0 + x, 5 + y]));
      ctx.moveTo(...proj(...ps)([x, y]));
      ctx.lineTo(...proj(...ps)([5 + x, 0 + y]));
      ctx.stroke();
