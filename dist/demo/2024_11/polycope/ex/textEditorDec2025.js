...
const least = (ar, f) => ar.reduce(
 ([b,pb,pbi],cs,i)=> f(cs)<pb ? [cs,f(cs),i] : [b,pb,pbi],
 [null,Infinity,null]
)
const sum = (ar) => ar.reduce(
 (s,cs)=> s+cs,
 0
)
...
const ins = (s, p, put) => s.slice(0,s.length-p)+put+s.slice(s.length-p)
const del = (s, p, n=1) => s.slice(0,s.length-p-n)+s.slice(s.length-p)
...
const ains = put=>actit(()=>{
 if (caret!==anchor) {
  s=del(s,Math.min(anchor,caret),Math.abs(anchor-caret))
  anchor=caret=Math.min(anchor,caret)
 }
 s=ins(s,caret,put)
})
const adel = ()=>actit(()=>{
