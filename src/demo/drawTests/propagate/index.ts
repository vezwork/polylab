...
const f = (func: Function, from: Datum, to: Datum = datum()) =>
...
const svg = document.getElementById("svg") as any as SVGElement;
...
function makeTextSvgEl(str: string, color: string) {
...
document.addEventListener("keydown", (e) => {
  ...
  f(
    ([k, [x, y]], next) => [next[0], [x + 10, y + Math.sin(x / 100) * 6]],
    keysHead,
    newKeyNode
  );
  const el = makeTextSvgEl(e.key, "black");
  f(([k, p]: [string, [number, number]]) => {
    el.textContent = k;
