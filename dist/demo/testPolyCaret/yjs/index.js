...
function refFrom(yArray, index) {
    const relI = index === undefined
        ? []
        : [Y.createRelativePositionFromTypeIndex(yArray, index)];
    if (yArray.parent === null)
        return { base: yArray, stack: relI };
    else {
        ...
        return {
            ...
            stack: [
                ...parent.stack,
                Y.createRelativePositionFromTypeIndex(yArray.parent, Array.from(yArray.parent).indexOf(yArray)),
                ...relI,
            ],
        };
    }
}
const indexStackFromRelStack = (relStack) => relStack.map((relPos) => Y.createAbsolutePositionFromRelativePosition(relPos, ydoc).index);
...
function delRef(ref) {
    ...
    const i = Y.createAbsolutePositionFromRelativePosition(ref.stack.at(-1), ydoc).index;
    p.delete(i - 1);
}
