import { V3, v3length, v3mul, v3normalize, v3rot } from "./v3.js";
...
export const qscalarMul = (n: number, a: Quat): Quat => [
  n * a[0],
  n * a[1],
  n * a[2],
  n * a[3],
...
export const qfromAxisAngle = (axis: V3, angle: number): Quat => [
  Math.cos(angle),
  ...v3mul(Math.sin(angle), axis),
];
export const qToAxisAngle = (a: Quat) => ({
  angle: Math.acos(a[0]),
  axis: v3mul(1 / Math.sin(Math.acos(a[0])), [a[1], a[2], a[3]]),
});

export const qconj = (a: Quat): Quat => [a[0], -a[1], -a[2], -a[3]];
export const qnorm = (a: Quat): number => Math.sqrt(qmul(a, qconj(a))[0]);
export const qnormalize = (a: Quat): Quat => qscalarMul(1 / qnorm(a), a);
export const qinv = (a: Quat): Quat =>
  qscalarMul(1 / qmul(a, qconj(a))[0], qconj(a));
const qbasisStrings = ["", "i", "j", "k"];
export const qToString = (a: Quat) =>
  a.reduce(
    (prev, cur, i) =>
      cur === 0
        ? prev
...
export const qFromV3 = (v: V3): Quat => [0, ...v];
export const qToV3 = (q: Quat): V3 => [q[1], q[2], q[3]];
export const qap =
  (q: Quat) =>
  (v: V3): V3 =>
    qToV3(qmul(qmul(q, qFromV3(v)), qinv(q)));
...
export const slerp = (a: Quat, b: Quat, t: number) => {
  ...
  return qscalarMul(
    1 / denom,
    qadd(
      qscalarMul(Math.sin((1 - t) * angle), a),
      qscalarMul(Math.sin(t * angle), b)
    )
  );
};
...
console.log(
  "qinv test!",
  qToString(qmul(qinv([-0.3, 1, -19, 1]), [-0.3, 1, -19, 1]))
);
const q = [0.27, 0.73, 0.61, 0.15] as Quat;
console.log(
  "q to axis angle",
  qfromAxisAngle([1, 0, 0], 0.13),
  "SHOULD BE [1, 0, 0], 0.13",
  qToAxisAngle(qfromAxisAngle([1, 0, 0], 0.13)),
  qToAxisAngle(q),
  "TEST!",
  v3length(qToAxisAngle(q).axis)
);

const testAxis = v3normalize([Math.random(), Math.random(), Math.random()]);
console.log(
  "qap test!",
  qap(qfromAxisAngle(testAxis, 0.13 / 2))([0.1, 0.1, 0.1]),
  v3rot(testAxis, 0.13)([0.1, 0.1, 0.1])
);
