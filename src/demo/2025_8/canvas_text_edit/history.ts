export const history = () => {
  ...
  const remove = (node) => {
    if (!node.parent) throw "attempting to remove root";
    node.removed = true;
    return parent;
  };
  const restore = (node) => {
  ...
  const redo = () => {
    if (historyHead.next.at(-1) === undefined) return undefined;
  ...
  const mainline = () => {
    ...
    let cur = historyHead;
    while (cur.parent) {
      if (cur === historyRoot) break;
      if (!cur.removed) result.unshift(cur.data);
      cur = cur.parent;
    }
    return result;
  };
