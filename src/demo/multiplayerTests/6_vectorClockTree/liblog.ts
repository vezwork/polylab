...
const entrywiseOneWayDiffPair = (myClock) =>
  objectLift((value: number, key) =>
    myClock[key] === undefined || value > myClock[key]
      ? [myClock[key], value]
      : undefined
  );

const clockDiff = (myClock, otherClock) =>
  removeUndefinedValues(entrywiseOneWayDiffPair(myClock)(otherClock));
...
function compareClocksAlphabetical(clock1, clock2) {
  ...
  for (const key of keys) {
    if ((clock1[key] ?? -1) > (clock2[key] ?? -1)) return 1;
    if ((clock1[key] ?? -1) < (clock2[key] ?? -1)) return -1;
  }
  return 0;
}
...
const addEvent = (id, ev, events) => {
  if (!Array.isArray(events[id])) events[id] = [];
...
export function setup(myName) {
  ...
  const requestDiff = async (diff) =>
    objectLift(([start, end]: any, key) =>
