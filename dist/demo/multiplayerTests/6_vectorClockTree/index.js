...
function initPeer(myId) {
    ...
    init();
    const { rootIndexPath, applyRootIndexPath, nodeAndAncestors } = makeTreeFunctions({
        parent: ({ parent }) => parent,
        children: ({ children }) => children,
    });
    const newHead = (v) => me.do({ v, id: myId, after: [...rootIndexPath(treeHeads[myId])] });
    const undo = () => {
    ...
    me.onUpdate((linearizedEvents) => {
        ...
        for (const { v, id, after, head } of linearizedEvents) {
            if (after) {
                ...
                const newNode = insertChild(applyRootIndexPath(treeRoots[id])(after))(v);
        ...
        div.textContent = linearizedEvents.reduce((acc, { v, id }) => {
            if (some(nodeAndAncestors(treeHeads[id]), ({ v: otherV }) => otherV === v)) {
                const { fn, key } = v;
                if (fn === "ins")
                    return acc + key;
                if (fn === "del")
