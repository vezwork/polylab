...
ws.addEventListener("open", () => {
    ...
    if (e.key.startsWith("Arrow")) {
      ...
      if ([...thru(my.anchor.at, my.caret.at)].length > 0) {
        ...
        if (to) {
          historyGroup(
            apiDel(my.caret.at),
            apiMoveCaret(my.caret, to, from),
            apiMoveCaret(my.anchor, to, from)
          );
        }
      }
      ...
      if ([...thru(my.anchor.at, my.caret.at)].length > 0) {
        ...
        historyGroup(
          delta,
          apiMoveCaret(my.caret, inserted),
          apiMoveCaret(my.anchor, inserted)
        );
      }
    }
    requestAnimationFrame(anim);
  ...
  function anim() {
    ...
    for (const { caret, anchor } of getLineCarets()) {
      for (const _ of map(
        thru(anchor.at, caret.at),
        drawSelectedCaretHost(ctx, caret.color)
      ));
    }
