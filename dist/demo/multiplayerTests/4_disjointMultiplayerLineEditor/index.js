...
const socketIdToCaretMap = new Map();
...
const unMoveCaret = (caret) => ({ dir, selection, isAnchorBeforeTow, }) => (s) => {
    if (caret.cur) {
...
const moveCaretSelecting = (caret) => (dir) => (s) => {
    if (caret.cur) {
        const next = to(s.hosts, caret.cur, dir, null, true);
...
const backspace = (caret) => () => (s) => {
    if (state.hosts.indexOf(caret.tow) !== -1)
        deleteSelection(caret)(s);
    else
        backspaceNoSelection(caret)(s);
};
...
const unbackspace = (caret) => ({ isAnchorBeforeTow, selection, cur, }) => (s) => {
    if (selection.length > 0) {
...
const uninsert = (caret) => ({ selection, isAnchorBeforeTow, }) => (s) => {
    backspace(caret)()(s);
    if (selection.length > 0) {
        insert(caret)(...selection)(s);
        const anchor = (isAnchorBeforeTow
...
function anim() {
    ...
    for (const caret of state.carets) {
        if (caret.cur) {
            ...
            ctx.strokeStyle = "red";
            ctx.strokeRect(getBounds(caret.cur).right, getBounds(caret.cur).top, 3, caret.cur.size);
        }
    }
...
const orderedSelectors = (caret) => (state) => !caret.anchor || !caret.tow
    ? null
...
function* traverseHosts(state, caret) {
    if (caret.anchor === null || caret.tow === null)
        return;
...
const isSelected = (caret) => (host) => some(traverseHosts(state, caret), (h) => h === host);
// history sketch
// Δ+a Δ+b Δ+c Δ+a Δ< Δ>
// Δ> Δ< = _
