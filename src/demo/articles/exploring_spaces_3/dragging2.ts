...
import { Vec2, angleOf } from "../../../lib/math/Vec2.js";
...
let state = {
  ...
  render() {
    ...
    for (const line of lines) {
      ...
      line?.setAttribute(
        "transform",
        `rotate(${(-pos).toFixed(3)}) translate(0 200) scale(${size.toFixed(
          3
        )})`
      );
      i++;
      line.querySelector("text").textContent = (i + Math.trunc(cx) - 1).toFixed(
        0
      );
    }
