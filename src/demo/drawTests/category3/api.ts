...
export const constructor = (fn: () => any, debugName?: string) => {
  const symbol = Symbol(debugName);
  symbolConstructor.set(symbol, fn);
  return symbol;
};

export const consume = (path: REdge[]) => connect.consume(path, words);
export const isConnected = (path: REdge[]) =>
  connect.queryConnectedness(words, simplify(rawPath(path)));
export const e =
  (from: symbol, to: symbol, path?: REdge[]) =>
  ({
    forward,
    backward,
  }: {
...
export const se =
  (leftFrom: symbol, rightFrom: symbol, to: symbol, rToLPath?: REdge[]) =>
  ({
    forward,
    backward,
  }: {
...
export function* propagateForward(
  start: symbol,
  startData = symbolConstructor.get(start)()
) {
  const init = create(start);
  const workingSplits = new Map<Thing, Map<Edge[], boolean>>();
        ...
        if (isSplitConnected) {
          ...
          if (workingSplits.get(to)?.get(split)) {
            ...
            const fromData = split.map((e) =>
              nodeData.get(applyPath(to, [inv(e)]))
            );
            fn(...fromData, toData);
            ...
            if (!workingSplits.has(to)) workingSplits.set(to, new Map());
...
export function* propagate(
  ...
  const visits = new Set<Thing>([init]);
  const workingSplits = new Map<Thing, Map<Edge[], boolean>>();

  const nodeData = new Map([[init, startData]]);
  const getData = (t: Thing) =>
    nodeData.get(t) ??
  ...
  const queue = [[init, []]] as [Thing, REdge[]][];
  ...
  while (queue.length !== 0) {
    ...
    for (const [edge, fromContainer] of currentNode.from.entries()) {
      ...
      if (split) {
        ...
        if (splitSources.some((s) => visits.has(s))) continue;
