...
import { add, rotate, setLength, sub } from "../../lib/math/Vec2.js";
...
const qnote = padD([10, 0])(caretable(pathD("M5.79325 12.3905C6.12789 13.3398 5.23798 14.5187 3.80559 15.0236C2.37319 15.5286 0.940723 15.1683 0.60608 14.2189C0.271437 13.2696 1.16134 12.0907 2.59374 11.5858C4.02614 11.0808 5.4586 11.4411 5.79325 12.3905ZM5.79325 12.3905V0.5", 7, 16, true, 2)));
...
const lntD = (text, size = 40) => centerBesideD(caretable(textD("", size, 1, size)), ...text
    .split("")
    .map((char) => caretable(textD(...measureWidth(char, size)))));
...
const frac = (slot1, slot2) => caretable(centerOverD(smallPad(slot1), overD(lx, smallPad(slot2))));
...
const capTernary = (cap) => (slot1, slot2, slot3) => caretable(neditCapTernary(cap)(slot1, slot2, slot3));
const Σ = capTernary("∑");
const Π = capTernary("∏");
...
const φ = centerBesideD(lntD("1 + "), caretable(centerOverD(lntD("1"), slad(placeholder))));
...
const { descendentsBreadthFirst: des, hasChildren, nodeAndAncestors, rootIndexPath, applyRootIndexPath, } = makeTreeFunctions({
    parent,
    children,
});
let carryX = null;
...
const citeImage = (url, cite, shoveX, additionalText = "") => translateD([shoveX, 0])(justOverD(textD(...measureWidth(cite, 3)), textD(...measureWidth(additionalText, 3)), caretable(scaleD([1 / 10, 1 / 10])(newImage(url)))));
const main = caretable(drawables(justOverD(tD("Thank you", 60), tD("elliot.website/editor", 30), padD([0, 170])(scaleD([300, 2])(lx)), tD("A Caret for your thoughts", 100), justBesideD(tD("Adapting Caret (", 50), caret, tD(") Navigation to Visual Editors", 50)), padD([0, 170])(scaleD([300, 2])(lx)), tD("Visual Editors                             "), padD([10, 20])(justOverD(citeImage("./img/cellpond.png", "Cellpond", 900, "Lu Wilson"), citeImage("./img/scratch.png", "Scratch Blocks", 700), citeImage("./img/livelits.png", "Filling Typed Holes with Live GUIs [LiveLits]", 500, "Cyrus Omar, David Moon, Andrew Blinn, Ian Voysey, Nick Collins, Ravi Chugh"), citeImage("./img/visr.jpg", "Adding Interactive Visual Syntax to Textual Code [VISr]", 300, "Leif Anderson, Michael Ballantyne, Matthias Felleisen"), citeImage("./img/mps.png", "jetbrains.com/mps/", 150), citeImage("./img/code.png", "Traditional Code Editor", 0))), padD([0, 170])(scaleD([300, 2])(lx)), centerBesideD(translateD([0, 220])(caretable(justOverD(tD("Text"), tD("Programming")))), newImage("./img/bridge.svg", 682, 420), translateD([0, -70])(caretable(justOverD(tD("Visual"), tD("Programming"))))), padD([0, 170])(scaleD([300, 2])(lx)), tD(`In Polytope we`, 50), tD(`can't navigate visuals`, 50), tD(`without using mouse`, 50), padD([0, 170])(scaleD([300, 2])(lx)), padD([30, 30])(tD("What should visual caret nav be?", 50)), padD([30, 30])(ntD("😌 feel text-editor-y", 40)), 
...
padD([0, 500])(caretable(centerOverD(tD("Visuals", 50), caretable(textD(...measureWidth("🤝", 50))), tD("Common controls", 50), caretable(textD(...measureWidth("🤝", 50))), tD("Text", 50)))), translateD([0, 300])(tD("Prior art", 50)), padD([30, 10])(ntD("🌐 W3C Spatial Nav", 30)), padD([30, 10])(ntD("💭 “Four-way navigation algorithm” stackoverflow.com/a/16577312", 30)), padD([30, 10])(centerBesideD(github, ntD(" “Spatial Keyboard Navigation” by danilowoz", 30))), padD([30, 10])(ntD("📺 Netflix Tech Blog “[..]User Input on TV Devices”", 30)), padD([30, 10])(ntD("𝐓 rich text editors (e.g. Google Docs, Notion, Word,...)", 30)), padD([0, 230])(scaleD([300, 2])(lx)), tD("What makes text editors so... text editor-y?"), padD([0, 80])(caretable(justOverD(lntD("function rgbFromNum(num: number) {", 30), lntD("  num >>>= 0;", 30), lntD("  const b = num & 0xff;", 30), lntD("  const g = (num & 0xff00) >>> 8;", 30), lntD("  const r = (num & 0xff0000) >>> 16;", 30), lntD("  return [r, g, b];", 30), lntD("}", 30)))), padD([0, 170])(scaleD([300, 2])(lx)), caretable(justOverD(centerBesideD(lntD("const mySum = "), nΣNote), centerBesideD(lntD("const gameJingle = "), scaleD([2, 2])(justBesideD(caretable(treble), translateD([0, 10])(qnote), translateD([0, -10])(qnote), translateD([0, 14])(qnote)))), centerBesideD(lntD("const myDiagram = "), translateD([50, 0])(drawables(arrow([90, 50], [90, 150]), translateD([80, 10])(lntD("n")), arrow([100, 50], [170, 250]), arrow([80, 50], [10, 250]), translateD([50, 155])(lntD("a×b")), arrow([100, 200], [150, 250]), arrow([80, 200], [30, 250]), translateD([170, 250])(lntD("b")), translateD([0, 250])(lntD("a"))))))), padD([0, 230])(scaleD([300, 2])(lx)), caretable(justOverD(centerBesideD(lntD("ζ(s)﹦"), ΣNote, lntD("﹦"), Π(lntD("∞"), lntD("p∊ℙ"), frac(lntD("1"), centerBesideD(lntD("1−"), exp(lntD("p"), lntD("−s")))))), padD([0, 120])(justBesideD(lntD("φ = "), transformD(id)(φ))), centerBesideD(integral(lx, lntD("∂Ω"), lntD("ω")), lntD("﹦"), integral(lx, lntD("Ω"), lntD("dω")), translateD([50, 0])(caretable(drawables(arrow([90, 50], [90, 150]), translateD([80, 10])(lntD("n")), arrow([100, 50], [170, 250]), arrow([80, 50], [10, 250]), translateD([50, 155])(lntD("a×b")), arrow([100, 200], [150, 250]), arrow([80, 200], [30, 250]), translateD([170, 250])(lntD("b")), translateD([0, 250])(lntD("a"))))))))), translateD([700, 1500])(tD(""))));
