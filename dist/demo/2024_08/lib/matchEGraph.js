...
import { allWaysToMergeOneFromEach, allWaysToMergeOneFromEachNoConflicts, } from "./matchHelpers.js";
export const eClassMatches = (patternNode, eClass) => [...items(eClass)].flatMap((eNode) => eNodeMatches(patternNode, eNode));
export const eNodeMatches = (patternNode, eNode) => {
    if (patternNode.var) {
        if (patternNode.value && eNode.value !== patternNode.value)
            return [];
        return successVarMatch(patternNode, eNode);
    }
    else if (patternNode.value) {
        if (eNode.value !== patternNode.value ||
            patternNode.children.length !== eNode.children.length)
            return [];
        else
            return successNodeMatch(patternNode, eNode);
    }
};
const successVarMatch = (patternNode, eNode) => allWaysToMergeOneFromEachNoConflicts([
    successNodeMatch(patternNode, eNode),
    [{ [patternNode.var]: eNode }],
]);
const successNodeMatch = (patternNode, eNode) => allWaysToMergeOneFromEachNoConflicts(patternNode.children.map((p, i) => eClassMatches(p, eNode.children[i])));
...
export const checkEq = (eClasses, ...patterns) => [...eClasses]
