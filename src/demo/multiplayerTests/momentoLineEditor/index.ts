...
let historyFocus: DoubleLinkedListNode<State> = {
  data: {
    cur: initCaret,
    anchor: null,
    tow: null,
...
const replace =
  ...
  (data: T) => {
    ...
    if (node?.prev?.next) node.prev.next = newNode;
    return newNode;
  };
const insertAfter =
...
const moveCaret = (next: CaretHost | null) => {
...
const moveCaretSelecting = (next: CaretHost | null) => {
  historyFocus = replace(historyFocus)({
    ...historyFocus.data,
...
const backspace = () => {
  if (historyFocus.data.anchor) deleteSelection();
  else backspaceNoSelection();
};
const noop = () => {
  historyFocus = insertAfter(historyFocus)({
...
const insert = (char: string) => {
  if (historyFocus.data.tow) deleteSelection();
  else noop();
...
const redo = () =>
  historyFocus.next ? (historyFocus = historyFocus.next) : undefined;
const undo = () =>
  historyFocus.prev ? (historyFocus = historyFocus.prev) : undefined;
...
addEventListener("mousedown", (e: MouseEvent) => {
  ...
  if (nextFocus) moveCaret(nextFocus);
});
addEventListener("mousemove", (e) => {
  if (e.buttons === 1) {
    const closest = closestTo(e.x * window.devicePixelRatio)(historyFocus.data);
    if (closest) return moveCaretSelecting(closest);
  }
});
addEventListener("keydown", (e) => {
  if (e.key.startsWith("Arrow")) {
    if (historyFocus.data.cur) {
      const nextFocus = to(
        historyFocus.data.hosts,
        historyFocus.data.cur,
        e.key as "ArrowUp" | "ArrowDown" | "ArrowLeft" | "ArrowRight",
        null,
        true
      );
      if (e.shiftKey) moveCaretSelecting(nextFocus);
      else moveCaret(nextFocus);
    }
  }
  ...
  if (e.key.length === 1) if (historyFocus.data.cur) return insert(e.key);
});
...
const orderOfCaretHosts = (a: CaretHost, b: CaretHost) =>
  historyFocus.data.hosts.indexOf(a) < historyFocus.data.hosts.indexOf(b);
...
const isSelected = (host: CaretHost) =>
  some(traverseHosts(historyFocus.data), (h) => h === host);
