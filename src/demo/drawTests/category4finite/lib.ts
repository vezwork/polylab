...
const cat = new SetMap<Object, [Function, Object]>();
const catop = new SetMap<Object, [Function, Object]>();
const op = new Map<[Function, Object], [Function, Object]>();

export const mo =
  (edge: Function) =>
  (from: any) =>
  (to: any = Symbol()) => {
    const forward = [edge, to] as [Function, Object];
    const backward = [edge, from] as [Function, Object];
  ...
  while (unvisitedProducts.size > 0) {
    ...
    while (queue.length > 0) {
      ...
      for (const forward of cat.get(from) ?? []) {
        ...
        const allForwardsToCurAreVisited = [...(catop.get(to) ?? [])].every(
          (backward) => visitedEdges.has(op.get(backward))
        );
