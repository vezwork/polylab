...
const arrow = (start: Vec2, end: Vec2) =>
  lineD([
    start,
    end,
...
const neditCapTernary =
  (cap: string) =>
  (slot1: BoundedDrawable, slot2: BoundedDrawable, slot3: BoundedDrawable) =>
    centerBesideD(centerOverD(slot1, lntD(cap, 100), slot2), slot3);
const capTernary =
  (cap: string) =>
  (slot1: BoundedDrawable, slot2: BoundedDrawable, slot3: BoundedDrawable) =>
...
const ΣNote = Σ(
  lntD("∞"),
  lntD("n﹦1"),
  frac(lntD("1"), exp(lntD("n"), lntD("s")))
);
const nΣNote = nΣ(
  lntD("∞"),
  lntD("n﹦1"),
  nfrac(lntD("1"), exp(lntD("n"), lntD("s")))
);
...
const { superParent, superChildren } = makeTreeFunctions<DrawTree>({
  parent: (e) => e.parent,
...
const newImage = (url: string, w = 0, h = 80) => {
  const img = new Image();
  fetch(url)
    .then((r) => r.blob())
  ...
  drawables(
    justOverD(
      ...
      justBesideD(
        ...
        caret,
        tD(") Navigation to Visual Editors", 50)
      ),
      padD([0, 170])(scaleD([300, 2])(lx)),
      tD("Visual Editors                             "),
      padD([10, 20])(
        justOverD(
          citeImage("./img/cellpond.png", "Cellpond", 900, "Lu Wilson"),
          citeImage("./img/scratch.png", "Scratch Blocks", 700),
          citeImage(
            "./img/livelits.png",
            "Filling Typed Holes with Live GUIs [LiveLits]",
            500,
            "Cyrus Omar, David Moon, Andrew Blinn, Ian Voysey, Nick Collins, Ravi Chugh"
          ),
          citeImage(
            "./img/visr.jpg",
            "Adding Interactive Visual Syntax to Textual Code [VISr]",
            300,
            "Leif Anderson, Michael Ballantyne, Matthias Felleisen"
          ),
          citeImage("./img/mps.png", "jetbrains.com/mps/", 150),
          citeImage("./img/code.png", "Traditional Code Editor", 0)
        )
      ),
      padD([0, 170])(scaleD([300, 2])(lx)),
      centerBesideD(
      ...
      tD(`can't navigate visuals`, 50),
      tD(`without using mouse`, 50),
      padD([0, 170])(scaleD([300, 2])(lx)),

      padD([30, 30])(tD("What should visual caret nav be?", 50)),
      padD([30, 30])(ntD("😌 feel text-editor-y", 40)),
      ...
      translateD([0, 300])(tD("Prior art", 50)),
      padD([30, 10])(ntD("🌐 W3C Spatial Nav", 30)),
      padD([30, 10])(
        ntD(
          "💭 “Four-way navigation algorithm” stackoverflow.com/a/16577312",
          30
        )
      ),
      padD([30, 10])(
        centerBesideD(
          github,
          ntD(" “Spatial Keyboard Navigation” by danilowoz", 30)
        )
      ),
      padD([30, 10])(
        ntD("📺 Netflix Tech Blog “[..]User Input on TV Devices”", 30)
      ),
      padD([30, 10])(
        ntD("𝐓 rich text editors (e.g. Google Docs, Notion, Word,...)", 30)
      ),
      ...
      padD([0, 80])(
        caretable(
          justOverD(
            ...
            lntD("  num >>>= 0;", 30),
            lntD("  const b = num & 0xff;", 30),
            lntD("  const g = (num & 0xff00) >>> 8;", 30),
            lntD("  const r = (num & 0xff0000) >>> 16;", 30),
            lntD("  return [r, g, b];", 30),
            lntD("}", 30)
          )
        )
      ),
      ...
      caretable(
        justOverD(
          ...
          centerBesideD(
            lntD("const myDiagram = "),
            translateD([50, 0])(
              drawables(
                arrow([90, 50], [90, 150]),
                translateD([80, 10])(lntD("n")),
                arrow([100, 50], [170, 250]),
                arrow([80, 50], [10, 250]),
                translateD([50, 155])(lntD("a×b")),
                arrow([100, 200], [150, 250]),
                arrow([80, 200], [30, 250]),
                translateD([170, 250])(lntD("b")),
                translateD([0, 250])(lntD("a"))
              )
            )
          )
        )
        ...
        justOverD(
          centerBesideD(
            ...
            Π(
              ...
              frac(
                lntD("1"),
                centerBesideD(lntD("1−"), exp(lntD("p"), lntD("−s")))
              )
            )
          ),

          padD([0, 120])(justBesideD(lntD("φ = "), transformD(id)(φ))),
          centerBesideD(
            integral(lx, lntD("∂Ω"), lntD("ω")),
            lntD("﹦"),
...
let curFocus: DrawTree = [...children(rend)][0];
document.body.addEventListener("keydown", (e) => {
  ...
  if (e.key === "d") drawBlueLines = !drawBlueLines;
...
function anim() {
  ...
  const a2 = _(scale([height / 80, height / 80]))(
    translation([height < 10 ? x - 4 + width : 0, y - 4])
  );
  ...
  const gradient = ctx.createRadialGradient(
    ...apply(_(caretTransform)(translation([20, 20])))([0, 0]),
    10,
    ...apply(_(caretTransform)(translation([20, 20])))([0, 0]),
    420
  );
  ...
  if (drawBlueLines) {
    ctx.fillStyle = "blue";
    canvasRender(
      translateD([c.width - 1050, c.height - 150])(ntD("Nesting", 150))
    );
  }
  if (drawLineOutlines) {
    ctx.fillStyle = "blue";
    canvasRender(
      translateD([c.width - 1050, c.height - 350])(ntD("Lines", 150))
    );
  }
  if (drawSinks) {
    ctx.fillStyle = "green";
    canvasRender(
      translateD([c.width - 1050, c.height - 550])(ntD("Caret Sinks", 150))
    );
  }
  if (drawAboveAndBelow) {
    ctx.fillStyle = "red";
    canvasRender(
      translateD([c.width - 1050, c.height - 750])(ntD("Above/", 150))
    );
    ctx.fillStyle = "purple";
    canvasRender(
      translateD([c.width - 520, c.height - 750])(ntD("Below", 150))
    );
  }
