...
document.getElementById("test")?.addEventListener("click", (e) => {
    ...
    setInterval(() => {
        ...
        if (Math.random() > 0.2)
            document.getElementById("me")?.dispatchEvent(new KeyboardEvent("keydown", {
        ...
        //   );
        if (Math.random() > 0.1)
            document.getElementById("me2")?.dispatchEvent(new KeyboardEvent("keydown", {
                key: "Backspace",
            }));
        if (Math.random() > 0.2)
            document.getElementById("me3")?.dispatchEvent(new KeyboardEvent("keydown", {
                key: "" + Math.round(Math.random() * 9),
            }));
    }, 300);
});
