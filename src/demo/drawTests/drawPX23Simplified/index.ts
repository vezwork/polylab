...
const { next, lines } = makeNestedCaretFunctions<DrawTree>({
  getBounds,
  getCarryX: () => carryX,
...
const linesFromChildren = (t: DrawTree) => lines(children(t));
...
let curFocus: DrawTree = first(children(rend))!;
...
function anim() {
  ...
  if (drawSinks) for (const t of des(rend)) drawBoundRight(ctx, t);
  if (drawAboveAndBelow) {
    const nexter = next(curFocus, "ArrowUp");
    if (nexter) drawLineBetweenBoundRights(ctx, curFocus, nexter, "red");
  ...
  const caretTransform = _(scale([height, height]))(
    translation([x - 4 + width, y - 4])
  );
