...
class Ref {
  constructor(public to: CaretHost) {}
  toJSON() {
    return { isRef: state.hosts.indexOf(this.to) };
  }
  static fromJSON(key: string, value: any) {
...
const parse = (data: any) =>
  JSON.parse(data, (key, value) => {
    ...
    if (refFrom !== undefined) return refFrom;
    return value;
  });
...
console.log(
  "parse stuff",
  JSON.stringify(new Ref(initCaret)),
  parse(JSON.stringify(new Ref(initCaret)))
);
...
const moveCaretOp = (arg: {
  dir: "ArrowUp" | "ArrowDown" | "ArrowLeft" | "ArrowRight";
  caret: CaretData;
}): Op => ({ function: "moveCaret", args: [arg] });
const moveCaret =
  ({
    dir,
    caret,
  ...
  (s: State) => {
    ...
    if (next !== null) caret.cur = s.hosts.indexOf(next);
    caret.anchor = caret.cur;
  };

const moveCaretSelectingOp = (arg: {
  dir: "ArrowUp" | "ArrowDown" | "ArrowLeft" | "ArrowRight";
  caret: CaretData;
}): Op => ({ function: "moveCaretSelecting", args: [arg] });
...
const backspaceΔ = (arg: { caret: CaretData }): Δ => {
...
const selectionOf = (caret: CaretData) => [...traverseHosts(state, caret)];
const backspace = (arg: { caret: CaretData }) => (s: State) => {
  if (selectionOf(arg.caret).length > 0) deleteSelection(arg)(s);
  else backspaceNoSelection(arg)(s);
};
const deleteSelection =
  ({ caret }: { caret: CaretData }) =>
  (s: State) => {
...
const backspaceNoSelection =
  ...
  (s: State): void => {
    const newHosts = [...s.hosts];
    if (caret.cur === 0) return; //hack so that initial thing cannot be deleted
...
const insert =
  ({ newCur, caret }: { newCur: CaretHost; caret: CaretData }) =>
  (s: State): void => {
...
const noop = () => (s) => s;
const Δregistry: {
  [key: string]: (...args: any) => (s: State) => void;
} = {
  noop,
  moveCaret,
  moveCaretSelecting,
...
addEventListener("keydown", (e) => {
  if (e.key.startsWith("Arrow")) {
    if (e.shiftKey)
      OpAp(
        moveCaretSelectingOp({
          dir: e.key as "ArrowUp" | "ArrowDown" | "ArrowLeft" | "ArrowRight",
          caret: myCaret,
        })
      );
    else
  ...
  if (e.key === "Backspace") return Δap(backspaceΔ({ caret: myCaret }));
...
function anim() {
  ...
  for (const caret of state.carets) {
    ...
    ctx.strokeStyle = "red";
    ctx.strokeRect(
      getBounds(state.hosts[caret.cur]).right,
      getBounds(state.hosts[caret.cur]).top,
      3,
      state.hosts[caret.cur].size
    );
  }
...
function* traverseHosts(state: State, caret: CaretData): Generator<CaretHost> {
  if (caret.anchor === caret.cur) return;
...
const isSelectedBy = (caret: CaretData) => (host: CaretHost) =>
