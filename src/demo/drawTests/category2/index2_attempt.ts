...
const arrow = (ob1, ob2) => {
  ...
  linkFn.set(pe1, (o1, { cur, prev }) => {
    if (prev[0] === undefined) prev[0] = _(id)(id);
    if (cur[0] !== undefined) assign(prev[0])(cur[0]);
    if (cur[0] === undefined) cur[0] = _(id)(id);
    if (approxEq(o1)(cur[0])) return false;
    assign(cur[0])(o1);
  });
  ...
  linkFn.set(pe2, (o2, { cur, prev }) => {
    if (prev[1] === undefined) prev[1] = _(id)(id);
    if (cur[1] !== undefined) assign(prev[1])(cur[1]);
    if (cur[1] === undefined) cur[1] = _(id)(id);
    if (approxEq(o2)(cur[1])) return false;
    assign(cur[1])(o2);
  });
  ...
  return [prod, pe1, pe2] as [symbol, Edge, Edge];
};
...
linkFn.set(e1, (t1, t2) => {
  ...
  if (approxEq(t2)(newT)) return false;
  assign(t2)(newT);
});
linkFnInv.set(e1, (t2, t1) => {
  const newT = _(t2)(tInv(botLeftT));
  if (approxEq(t1)(newT)) return false;
  assign(t1)(newT);
});
...
linkFn.set(e2, (t1, t2) => {
  ...
  if (approxEq(newT)(t2)) return false;
  assign(t2)(newT);
});
linkFnInv.set(e2, (t2, t1) => {
  const newT = _(t2)(tInv(botRightT));
  if (approxEq(newT)(t1)) return false;
  assign(t1)(newT);
});
...
linkFn.set(ar1svgE, ({ cur }, svg) => {
  if (cur[0] === undefined || cur[1] === undefined) return false;
...
function* propagateForward(
  start,
  startData = symbolConstructor.get(start.symbol)()
) {
...
function* propagate(start, startData = symbolConstructor.get(start.symbol)()) {
  ...
  while (queue.length !== 0) {
    ...
    for (const [edge, fromContainer] of currentNode.from.entries()) {
      ...
      if (optionalPropagate !== false) queue.push(from);
    }
...
//   why?
//   - need the code to be heavily simplified so I can add more complex features
//   alternatively I can just abstract away the isomorphisms but it will be messier.
